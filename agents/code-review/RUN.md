...
1. Do **not** preload an architect skill — the stack is unknown until the PR is inspected. The right skill is loaded in step 3 of *Executing the Task*.
...
3. **Load the matching architect skill** — `react-architect`, `fastapi-rules`, or both for mixed PRs. If the stack is unknown, signal `RUN_NEEDS_INPUT` and stop.
