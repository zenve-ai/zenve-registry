...
5. **Post the review.**
   ...
   - Required: exactly one `gh pr review <N> --approve|--request-changes|--comment --body "..."` as the final review action.
...
3. Confirm exactly one `gh pr review <N>` call was made (or that you are signaling `RUN_NEEDS_INPUT` / `RUN_FAILED` and have not posted a review).
...
| `RUN_CHANGES_REQUESTED: <note>` | Blockers found; review posted with `--request-changes` |
