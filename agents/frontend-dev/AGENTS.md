...
- **A direct task message** — e.g. "create a hello world page". Just implement it; there is no issue to fetch.
...
If the task is ambiguous, comments conflict with each other or with the description, or the work requires backend changes outside your scope, stop: post a comment on the issue describing the specific blocker (via `zenve-issues`), leave the issue **open**, and return `RUN_NEEDS_INPUT`.
